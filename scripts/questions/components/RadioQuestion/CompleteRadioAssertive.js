...
function generateCompleteRadioAssertive(assertive, questionId, currentAnswer) {
  ...
  const radioAssertiveContainer = createElementWithClasses(
    "div",
  ...
  assertiveStructure.map((assertiveItem, assertiveIndex) => {
    ...
    if (shouldRenderInput) {
      const input = createElementWithClasses(
        "input",
        "w-32 bg-inherit text-secondary-300 font-semibold border-b-2 border-secondary-500 outline-none mx-1 text-center"
      ); // bg-inherit added
...
export { generateCompleteRadioAssertive };
