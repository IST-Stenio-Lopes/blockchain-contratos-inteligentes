...
function loadPage() {
  ...
  currentPage = doLMSGetLessonLocation() || 1;
  lessonStatus = doLMSGetLessonStatus() || 1;
...
function loadData() {
  ...
  try {
    let susData = doLMSGetSuspendData();
    data = JSON.parse(susData);
  } catch (error) {
    console.log(error);
  }
  ...
  ueData.currentPage = Number(currentPage);

  return ueData;
}

function updateLessonPage(value) {
  currentPage = value.currentPage;
  doLMSSetLessonLocation(currentPage);

  if (lessonStatus != "completed" && currentPage >= ueData.totalOfPages) {
  ...
  let data = {
    currentPage,
    totalOfPages: value.totalOfPages,
  ...
  if (currentPage >= ueData.lastPageSeen) {
    data.lastPageSeen = currentPage;
  }

  ueData = data;

  doLMSSetSuspendData(JSON.stringify(data));
}
